...
const create_new_contact_modal = document.getElementById(
...
create_new_contact.addEventListener("click", () => {
  ...
  if (activeGroup == null) activeGroup = "None";
...
const saveContact = event => {
  const newContact = {
    ...
    email: `${document
      .getElementById("email-new-contact")
      .value.toLowerCase()}`,
    ...
    photo: `${getPhotoPath(
      document.getElementById("photo-new-contact").value
    )}`,
...
const saveEditedContact = address => {
  ...
  address_book[index].phone = document.getElementById(
    "phone-edit-contact"
  ).value;
  address_book[index].email = document
    .getElementById("email-edit-contact")
    .value.toLowerCase();
  address_book[index].group = document.getElementById(
    "group-edit-contact"
  ).value;
  address_book[index].address = document.getElementById(
    "address-edit-contact"
  ).value;
  address_book[index].birth_day = document.getElementById(
    "birth-day-edit-contact"
  ).value;
  const editedPhoto = `${getPhotoPath(
    document.getElementById("photo-edit-contact").value
  )}`;
  if (editedPhoto != "null") address_book[index].photo = editedPhoto;
  init(address_book);
...
window.addEventListener("click", event => {
  if (event.target === create_new_contact_modal)
    closeModal(create_new_contact_modal);
  else if (event.target === view_contact_modal) closeModal(view_contact_modal);
  else if (event.target === editContactModal) closeModal(editContactModal);
});
...
function handleAddressEdit(addressID) {
  ...
  editContactModal.children[0].innerHTML = `<h2>Edit Contact</h2>
    <div class="modal-flex-area">
        <div class="flex-item-1">
            <img src=${
              address.photo !== "null"
        ...
        <div class="flex-item-2">
            <form onsubmit="saveEditedContact(${addressID}); return false;">
                <table>
                    <tr>
                        <td class="col1"><label for="name-edit-contact">Name</label></td>
                        <td class="col2"><input type="text" id="name-edit-contact" name="Name" value='${
                          address.name
                        }' required></td>
                    </tr>
                    <tr>
                        <td class="col1"><label for="phone-edit-contact">Phone</label></td>
                        <td class="col2"><input type="tel" id="phone-edit-contact" name="Phone" pattern="[1-9]{1}[0-9]{9}" value=${
                          address.phone
                        }></td>
                    </tr>
                    <tr>
                        <td class="col1"><label for="email-edit-contact">Email</label></td>
                        <td class="col2"><input type="email" id="email-edit-contact" name="Email" value=${
                          address.email
                        }></td>
                    </tr>
                    <tr>
                        ...
                        <td class="col2"><select id="group-edit-contact" name="Group">
                            <option value="None" ${
                              "None" == address.group ? "selected" : ""
                            }>None</option>
                            <option value="Favourites" ${
                              "Favourites" == address.group ? "selected" : ""
                            }>Favourites</option>
                            <option value="Home" ${
                              "Home" == address.group ? "selected" : ""
                            }>Home</option>
                            <option value="Work" ${
                              "Work" == address.group ? "selected" : ""
                            }>Work</option>
                        </select></td>
                    </tr>
                    <tr>
                        <td class="col1"><label for="birth-day-edit-contact">Birthday</label></td>
                        <td class="col2"><input type="date" id="birth-day-edit-contact" name="Birthday" value=${
                          address.birth_day
                        }                  
                    </tr>
                    <tr>
                        <td class="col1"><label for="address-edit-contact">Address</label></td>
                        <td class="col2"><textarea id="address-edit-contact" name="Address">${
                          address.address
                        }</textarea></td>
                    </tr>
                    <tr>
    ...
    timeout = setTimeout(previous => {
      ...
      const matchedAddress = address_book.filter(address => {
        if (address.name.toLowerCase().search(name.toLowerCase()) !== -1)
          ...
          return true;
        else if (address.phone.search(name.toLowerCase()) !== -1) return true;
        else return false;
      });
      init(matchedAddress);
    }, 100);
...
function activeGroupGenerator() {
  ...
  if (activeGroupList.length == 0) return null;
  else {
...
function activeGroupListGenerator(addressBook) {
  ...
  if (activeGroup == null) return addressBook;
  else {
    return addressBook.filter(address => {
      if (address.group.includes(activeGroup)) return true;
      else return false;
    });
  }
}
...
function handleAddressOptions(event) {
  if (event.target.id == "placard-delete")
    ...
    event.target.id == "group-initial" ||
    event.target.id == "group-initial-h2" ||
    event.target.id == "group-initial-span"
  ) {
  } else displayContact(event.currentTarget.id);
}
...
function renderAddressBook(address_book) {
  address_book.forEach(element => {
    ...
    address.innerHTML = `
            <div id="group-initial" class="group-initial tooltip" title="Group: ${
              element.group
            }">
                <h2 id="group-initial-h2">${getInitials(element.group)}</h2>
            </div>
            <div class="profile" style="background-color: ${
            ...
            <div class="profile-photo" 
                style="background-image: url('${
                  element.photo != "null" ? element.photo : "ea"
